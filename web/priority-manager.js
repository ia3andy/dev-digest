document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Read tracker (runs on all pages with read buttons) ---

  const READ_KEY = 'digest-read-posts';
  const readState = JSON.parse(localStorage.getItem(READ_KEY) || '{}');

  function isRead(date) {
    if (readState[date] === false) return false;
    return readState[date] === true || !!(readState.readBefore && date <= readState.readBefore);
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (btn.classList.contains('digest-btn-mark-all-read')) {
      markAllRead(date);
    } else {
      toggleRead(date);
    }
  ...
  // Auto-read on detail page after scrolling 200px into the article
  ...
  // --- Priority rendering (only on digest article pages) ---
